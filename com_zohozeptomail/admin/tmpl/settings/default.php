  ...
  if (isset($_POST['zeptomail_submit'])) {
		...
		}else if ( $need_to_test == 1)
		{
			...
			if ($mailSent) {
				...
				JFactory::getApplication()->enqueueMessage(JTEXT::_('COM_ZOHOZEPTOMAIL_CONFIGURE_SUCCESS'),"message");
			} else
			{
				$data = json_decode($mailresp);
				$message_key= 'COM_ZOHOZEPTOMAIL_VALID_TOKEN';
				if(!empty($data->error)) {
					...
					if(!empty($data->error->details[0]->target) && strcmp($data->error->details[0]->target,"bounce_address") == 0 ) {
						$message_key = "COM_ZOHOZEPTOMAIL_VALID_BOUNCEADDR";
					}
					if(!empty($data->error->details[0]->target) && strcmp($data->error->details[0]->target,"from") == 0 ) {
						$message_key = "COM_ZOHOZEPTOMAIL_VALID_FROMADDR";
					}
				}
...
<div class="page"><div class="page__content">
    ...
    <form method="post" action="<?php echo $_SERVER["REQUEST_URI"]; ?>">
            ...
            <div class="form">
                <div class="form__row">
                    <label class="form--label"><?php echo JText::_('COM_ZOHOZEPTOMAIL_LABEL_HOST'); ?></label>
                    ...
                    </select> <br><i class="form__row-info"><?php echo JText::_('COM_ZOHOZEPTOMAIL_INFO_HOST'); ?></i> </div>
                    <div class="form__row">
                        <label class="form--label"><?php echo JText::_('COM_ZOHOZEPTOMAIL_LABEL_TOKEN'); ?></label>
                        ...
                        <i class="form__row-info"><?php echo JText::_('COM_ZOHOZEPTOMAIL_INFO_TOKEN'); ?></i>
                    </div>
                    <div class="form__row">
                        <label class="form--label"><?php echo JText::_('COM_ZOHOZEPTOMAIL_LABEL_BOUNCE'); ?></label>
                        ...
                        <i class="form__row-info"><?php echo JText::_('COM_ZOHOZEPTOMAIL_INFO_BOUNCE'); ?></i>
					</div>
                    <div class="form__row">
						<label class="form--label"><?php echo JText::_('COM_ZOHOZEPTOMAIL_LABEL_FROM'); ?></label>
						...
						<i class="form__row-info"><?php echo JText::_('COM_ZOHOZEPTOMAIL_INFO_FROM'); ?></i>
					</div>
                    <div class="form__row">
						<label class="form--label"><?php echo JText::_('COM_ZOHOZEPTOMAIL_LABEL_FROMNAME'); ?></label>
						...
						<i class="form__row-info"><?php echo JText::_('COM_ZOHOZEPTOMAIL_INFO_FROMNAME'); ?></i>
					</div>


					<div class="form__row form__row-btn">
						<input type="submit" name="zeptomail_submit" id="zeptomail_submit" class="btn" value="<?php echo JText::_('COM_ZOHOZEPTOMAIL_LABEL_SAVE'); ?>"/>
                    </div>
                </div>
    </form>
