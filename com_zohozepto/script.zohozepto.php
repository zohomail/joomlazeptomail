 ...
 *
 */
class Com_ZohoZeptoInstallerScript extends InstallerScript {
	...
	public function install( $parent ) {	
		...
		if(!$this->isConfigured()) {
		    echo JText::_('COM_ZOHOZEPTO_NOCONFIG');
		}
	}
			...
			foreach ( $plugins->children() as $plugin )	{
				...
				if ( ! empty( $extension ) ) {
					...
					if ( $result ) {
						$app->enqueueMessage( 
							Text::sprintf(
								'COM_ZOHOZEPTO_PLUGIN_UNINSTALL_SUCCESS',
								$pluginGroup,
								$pluginName
							)
						 );
						...
						$app->enqueueMessage(
							Text::sprintf(
								'COM_ZOHOZEPTO_PLUGIN_UNINSTALL_FAILURE',
								$pluginGroup,
								$pluginName
							),
							'error'
