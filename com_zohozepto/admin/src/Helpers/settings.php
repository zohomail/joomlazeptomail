 ...
 */
namespace Zoho\Component\ZohoZepto\Administrator\Helpers;
...
class SettingsHelper {
	...
	public static function configure_zeptomail($postData) {
		...
		$params = \JComponentHelper::getParams('com_zohozepto');
		$input = $appl->input;
    ...
    $query->where($db->quoteName('name') . ' = \'Zoho Zepto\'');
	$db->setQuery($query);
    ...
    $conditions = array(
        $db->quoteName('name') . ' = ' . $db->quote("Zoho Zepto")
    );
	...
	public static function sendTestMail($to,$subject,$content,$mail_type) {
		...
		if(empty($pluginParams->get('zeptomail_send_mail_token'))){
		    $appl->enqueueMessage(Text::_('COM_ZOHOZEPTO_NOCONFIG'),'error');
		    return false;
		}
		
		if (!filter_var($to, FILTER_VALIDATE_EMAIL)) {
          $appl->enqueueMessage(Text::_('COM_ZOHOZEPTO_ENTER_RECIPIENT'),'error');
          return false;
        }
        if(empty(trim($subject))) {
            $appl->enqueueMessage(Text::_("COM_ZOHOZEPTO_SPECIFY_SUBJECT"),'error');
            return false;
        }
		$mailer->addRecipient($to);
		if(empty(trim($content))) {
		    $appl->enqueueMessage(Text::_("COM_ZOHOZEPTO_SPECIFY_CONTENT"),'error');
		    return false;
		}
		...
		if($mailer->Send()){
			$appl->enqueueMessage(Text::_("COM_ZOHOZEPTO_MAIL_SENT"),'message');
			return true;
		}
		$appl->enqueueMessage(Text::_("COM_ZOHOZEPTO_MAIL_NOT_SENT"),'error');
		return false;
	}
